...
<?php
	...
	if (mysqli_connect_errno())	{
  		...
  		if ($_REQUEST['titol'].$_REQUEST['autoria'].$_REQUEST['materies'].
			...
			$textconsulta = "SELECT DISTINCT fitxa.id_fitxa, autoria, data, titol, edicio, editor_literari, lloc, editorial ".
				"FROM fitxa ".$clausula.";";
			//echo $textconsulta;
	...
	function construeix_clausula_cerca() {
		...
		if ($_REQUEST['id_exemplar'] != "") {
			...
			if ($_REQUEST['cdu'] != "")
			{
				$rossega = " INNER JOIN exemplar ON fitxa.id_fitxa=exemplar.id_fitxa ";
			}
			$rossega .= " WHERE ";
			$addicional = "";
		}
